import type { SalesOrder, Customer, OrderLine } from "@/types/sales-order"
...
export function InvoicePreview({
  ...
  const { showCustomerInfo, showOrderLines, showTotals, showNotes, showCurrency, currency } = config
  return (
            ...
            <tbody>
              {order.orderLines?.map((line: OrderLine, index: number) => (
                <tr key={line.product_id || index} className="border-b">
                  ...
                  <td className="text-right py-3 px-4">${line.product_price.toFixed(2)}</td>
                  <td className="text-right py-3 px-4">${line.total.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end">
            <div className="w-64">
              ...
              <div className="flex justify-between py-3 border-t font-semibold">
                <span>Total:</span>
                <span>
                  {showCurrency && `${currency} `}${order.total?.toFixed(2) || "0.00"}
                </span>
              </div>
            </div>
          </div>
